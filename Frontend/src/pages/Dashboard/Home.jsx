...
const Home = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      // Use dashboard API endpoint which handles all data correctly
      const response = await axiosInstance.get(API_PATH.DASHBOARD.GET_DATA)
      
      if (response.data.success) {
        // Backend already calculated everything correctly
        setData({
          pendingTasksToday: response.data.data?.pendingTasksToday || [],
          completedTasksToday: response.data.data?.completedTasksToday || [],
          overdueTasks: response.data.data?.overdueTasks || [],
        ...
        setDashboardStats({
          ...
          today: response.data.today || { pending: 0, completed: 0 },
          overdue: response.data.overdue || { all: 0 },
          last30DaysPerformance: response.data.last30DaysPerformance || []
        })
      }
    } catch (err) {
            ...
            <div className="p-3 sm:p-4 md:p-6">
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-base sm:text-lg md:text-xl font-bold" style={{ color: 'oklch(0.15 0.06 245)' }}>
                     Performance of last 30 days
                  </h3>
                </div>
                {/* Legend */}
                <div className="flex gap-3 sm:gap-4 text-xs sm:text-sm p-2 rounded-lg flex-wrap" style={{ backgroundColor: 'white' }}>
                  <div className="flex items-center gap-1.5">
                    <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded" style={{ backgroundColor: 'oklch(0.5 0.06 160)' }}></div>
                    <span style={{ color: 'oklch(0.4 0.06 245)' }}>Completed</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded" style={{ backgroundColor: 'oklch(0.5 0.06 100)' }}></div>
                    <span style={{ color: 'oklch(0.4 0.06 245)' }}>Pending</span>
                  </div>
                </div>
              </div>
              <div className="h-56 sm:h-72 md:h-96 lg:h-[28rem] w-full min-h-56">
